...
1. **The final summary report is generated by the critic agent** after validating all findings from the analysis agents (risk-analyzer, etc.), filtering false positives, resolving contradictions, and synthesizing actionable recommendations with a clear merge decision (SAFE TO MERGE or DO NOT MERGE).

2. **The report is output as structured Markdown** to `output/pr-{NUMBER}/final-report.md`, containing severity-ranked findings with file:line references, specific test recommendations, impact analysis, and action items—serving as the single source of truth for production readiness assessment.
